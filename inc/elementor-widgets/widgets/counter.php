 ...
 * @since 1.0
 */
class Ariclaw_Counter extends Widget_Base {

	public function get_name() {
		return 'ariclaw-counter';
	}

	public function get_title() {
		return __( 'Counter', 'ariclaw' );
	}
    ...
    public function ariclaw_get_svg_icon( $icon_name ){
        switch ( $icon_name ) {
            case 'cap':
                $icon_name = ARICLAW_DIR_ICON_IMG_URI .'cap.svg';
                break;
            case 'bag':
                $icon_name = ARICLAW_DIR_ICON_IMG_URI .'bag.svg';
                break;
            case 'shirt':
                $icon_name = ARICLAW_DIR_ICON_IMG_URI .'shirt.svg';
                break;
            
            default:
                $icon_name = ARICLAW_DIR_ICON_IMG_URI .'cafe.svg';
                break;
        }
	...
	protected function _register_controls() {
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Counter Setting', 'ariclaw' ),
			]
		);
		...
		$this->add_control(
            'counter_contents', [
                ...
                'fields' => [
                    [
                        'name'      => 'icon_img',
                        'label'     => __( 'Counter Image', 'ariclaw' ),
                    ...
                    [
                        'name'  => 'count_val',
                        'label' => __( 'Counter Value', 'ariclaw' ),
        ...
        $this->add_control(
            'counter_color', [
                'label'     => __( 'Counter Text Color', 'ariclaw' ),
            ...
            <div class="row">
                ...
                if( is_array( $counter_contents ) && count( $counter_contents ) > 0 ){
                    foreach ($counter_contents as $counter ) {
                        $icon_img = !empty( $counter['icon_img'] ) ? $this->ariclaw_get_svg_icon( $counter['icon_img'] ) : '';
